...
def train_PINNs(PINNs, train_data, t_train, val_data,t_val, epochs, optimizer):
    ...
    for epoch in range(epochs):
        # PINNs.train()
        train_loss, _, _= loss_function(PINNs, train_data,  t_train)
        optimizer.zero_grad()
        train_loss.backward()
        optimizer.step()
        trainloss.append(train_loss)
        # PINNs.eval()
        val_loss, _, _= loss_function(PINNs, val_data,  t_val)
        optimizer.zero_grad()
        val_loss.backward()
        optimizer.step()
...
trainl, vall, t_time=train_PINNs(PINNs, train_data, t_train, val_data,t_val, epochs, optimizer)
