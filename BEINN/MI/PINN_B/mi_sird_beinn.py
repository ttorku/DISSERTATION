...
class SIRD:
    def __init__(self, I, R, S,D, T,N0, layers, eps):
        self.t = T
        self.I = I
        self.R = R
        self.S = S
        ...
        # self.los = self.lossData + self.lossPhy
        # Calculate the regularization term
        # regularization_loss = eps * tf.reduce_sum([tf.nn.l2_loss(var) for var in tf.trainable_variables()])
        # # Add the regularization term to the original loss
        # self.loss = self.los + regularization_loss
        '''
    ...
    def attention(self, x):
        ...
        attention_weights = tf.reduce_sum(tf.abs(x), axis=1, keepdims=True)
    ...
    def neural_net1(self, X, weights, biases):
        num_layers = len(weights) + 1
        T= 2.0*(X - self.lb)/(self.ub - self.lb) - 1.0
        HH= tf.cast(T, dtype=tf.float32)
        H = self.attention(HH)
        for l in range(0,num_layers-2):
            W = weights[l]
            b = biases[l]
...
start_time = time.time()
model = SIRD(II/N0,RR/N0,SS/N0,DD/N0, T,1,layers, eps)
model.train(40000)
# End time
end_time = time.time()
...
np.savetxt(out +'S_true_{}_{}.txt'.format(case, eps),S_original.flatten().tolist())
np.savetxt(out +'I_true_{}_{}.txt'.format(case, eps),I_original.flatten().tolist())
np.savetxt(out +'R_true_{}_{}.txt'.format(case, eps),R_original.flatten().tolist())
np.savetxt(out +'D_true_{}_{}.txt'.format(case, eps),D_original.flatten().tolist())
np.savetxt(out +'S_pred_{}_{}.txt'.format(case, eps),s_p.flatten().tolist())
np.savetxt(out +'I_pred_{}_{}.txt'.format(case, eps),i_p.flatten().tolist())
np.savetxt(out +'R_pred_{}_{}.txt'.format(case, eps),r_p.flatten().tolist())
np.savetxt(out +'D_pred_{}_{}.txt'.format(case, eps),d_p.flatten().tolist())
#parameters
np.savetxt(out +'beta_pred_{}_{}.txt'.format(case, eps),beta_p.flatten().tolist())
np.savetxt(out +'gamma_pred_{}_{}.txt'.format(case, eps),gamma_p.flatten().tolist())
np.savetxt(out +'delta_pred_{}_{}.txt'.format(case, eps),delta_p.flatten().tolist())
np.savetxt(out +'compliance_pred_{}_{}.txt'.format(case, eps),compliance_p.flatten().tolist())
#loss
np.savetxt(out +'lossData_{}_{}.txt'.format(case, eps),model.loss_data)
np.savetxt(out +'lossPhy_{}_{}.txt'.format(case, eps),model.loss_phys)
np.savetxt(out +'total_loss_{}_{}.txt'.format(case, eps),model.total_loss)
...
#Error Metrics
with open(out+'MIbeinn.txt', 'w') as f:
