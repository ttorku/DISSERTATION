...
def train_PINNs(PINNs, train_data, t_train, epochs, optimizer, eps):
    ...
    for epoch in range(epochs):
        # PINNs.train()
        total_loss, _, _= loss_function(PINNs, train_data,  t_train, eps)
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
        ...
        if epoch % 500== 0:
            elapsed = time.time() - start_time
            print(f"Epoch: {epoch}, Total Loss: {total_loss.item():.5f}, Time: {elapsed:.2f} seconds")
            start_time = time.time()
    return trainloss, totaltime
...
data =torch.hstack((SS/N0, II/N0, RR/N0, DD/N0)).to(device)
t=t.to(device)
train_data=data
t_train=t
start_time = time.time()
trainl,_=train_PINNs(PINNs, train_data, t_train, epochs, optimizer, eps)
elapsed = time.time() - start_time
total_time =elapsed 
...
D_original=DD_/N0
case ='c1'
np.savetxt(out +'S_truebinn_{}_{}.txt'.format(case, eps),S_original.flatten().tolist())
np.savetxt(out +'I_truebinn_{}_{}.txt'.format(case, eps),I_original.flatten().tolist())
np.savetxt(out +'R_truebinn_{}_{}.txt'.format(case, eps),R_original.flatten().tolist())
np.savetxt(out +'D_truebinn_{}_{}.txt'.format(case, eps),D_original.flatten().tolist())
np.savetxt(out +'S_predbinn_{}_{}.txt'.format(case, eps),s_p.flatten().tolist())
np.savetxt(out +'I_predbinn_{}_{}.txt'.format(case, eps),i_p.flatten().tolist())
np.savetxt(out +'R_predbinn_{}_{}.txt'.format(case, eps),r_p.flatten().tolist())
np.savetxt(out +'D_predbinn_{}_{}.txt'.format(case, eps),d_p.flatten().tolist())
#parameters
np.savetxt(out +'beta_predbinn_{}_{}.txt'.format(case, eps),beta_p.flatten().tolist())
np.savetxt(out +'gamma_predbinn_{}_{}.txt'.format(case, eps),gamma_p.flatten().tolist())
np.savetxt(out +'delta_predbinn_{}_{}.txt'.format(case, eps),delta_p.flatten().tolist())
np.savetxt(out +'compliance_predbinn_{}_{}.txt'.format(case, eps),compliance_p.flatten().tolist())
#loss
np.savetxt(out +'Trainlossbinn_{}_{}.txt'.format(case, eps),trainl)
# np.savetxt(out +'Vallossbinn_{}_{}.txt'.format(case, eps),vall)
