...
def loss_function(PINNs, y_true,  t):
    ...
    overall_loss = lossData + lossPhy
    return overall_loss, lossPhy, lossData

def train_PINNs(PINNs, train_data, t_train, epochs, optimizer):
    ...
    for epoch in range(epochs):
        # PINNs.train()
        total_loss, _, _= loss_function(PINNs, train_data,  t_train)
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
...
trainl, t_time=train_PINNs(PINNs, train_data, t_train, epochs, optimizer)
...
#loss
np.savetxt(out +'Trainlosspinn_{}.txt'.format(case),trainl)
# np.savetxt(out +'Vallosspinn_{}.txt'.format(case),vall)
...
#Error metrics
with open(out+'PINNout.txt', 'w') as f:
    print("=======================================================\n", file=f)
