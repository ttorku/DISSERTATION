...
class SIRD:
    ...
    def net_param(self, t):
        NN=self.N
        beta= self.neural_net(t, self.weights5, self.biases5)
...
T = T.reshape(len(T),1)
layers = [1,64,64,64,64,1] #case 3
# layers = [1,62,62,62,62,1] #case 1, 2, 3
...
np.savetxt(out +'S_true_{}.txt'.format(case),S_original.flatten().tolist())
np.savetxt(out +'I_true_{}.txt'.format(case),I_original.flatten().tolist())
np.savetxt(out +'R_true_{}.txt'.format(case),R_original.flatten().tolist())
np.savetxt(out +'D_true_{}.txt'.format(case),D_original.flatten().tolist())
np.savetxt(out +'S_pred_{}.txt'.format(case),s_p.flatten().tolist())
np.savetxt(out +'I_pred_{}.txt'.format(case),i_p.flatten().tolist())
np.savetxt(out +'R_pred_{}.txt'.format(case),r_p.flatten().tolist())
np.savetxt(out +'D_pred_{}.txt'.format(case),d_p.flatten().tolist())
#parameters
np.savetxt(out +'beta_pred_{}.txt'.format(case),beta_p.flatten().tolist())
np.savetxt(out +'gamma_pred_{}.txt'.format(case),gamma_p.flatten().tolist())
np.savetxt(out +'delta_pred_{}.txt'.format(case),delta_p.flatten().tolist())
np.savetxt(out +'compliance_pred_{}.txt'.format(case),compliance_p.flatten().tolist())
#loss
np.savetxt(out +'lossData_{}.txt'.format(case),model.loss_data)
np.savetxt(out +'lossPhy_{}.txt'.format(case),model.loss_phys)
np.savetxt(out +'total_loss_{}.txt'.format(case),model.total_loss)
