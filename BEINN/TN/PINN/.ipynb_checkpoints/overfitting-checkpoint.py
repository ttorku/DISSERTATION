...
for i in range(3):
    ...
    np.savetxt(out +'loss_trainpinn_{}_{}.txt'.format(epochs[i],l), model.total_loss) ##train loss
    #val set
    lb1 =209 #change this
    ub1 =280
    length1=int(ub1-lb1)
    ...
    model1 = SIRD(II1/N0,RR1/N0,SS1/N0,DD1/N0, T1,1,layers)
    model1.train(epochs[i])
    np.savetxt(out +'loss_valpinn_{}_{}.txt'.format(epochs[i],l), model1.total_loss) ##train loss
    ...
    #Error Metrics
    with open(out+'outputpinn_{}_{}.txt'.format(epochs[i], l), 'w') as f:
        print("Error metrics for  epochs={} and epsilon={}".format(epochs[i], l), file=f)
        print("=======================================================\n", file=f)
